    ...
    <header className="sticky top-0 z-50 w-full bg-white px-5">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          {/* Modal Panel */}
          <div className="absolute left-1/2 top-8 w-[90%] max-w-[420px] -translate-x-1/2 rounded-2xl bg-white p-6 shadow-xl">
            {/* Close Button */}
            <button
              onClick={() => setMobileOpen(false)}
                ...
                <div key={item.label}>
                  {item.children ? (
                    <button
                      ...
                      className="flex w-full items-center justify-between py-3 font-outfit text-lg font-normal text-primary"
                    >
                      {item.label}
                      <svg
                    ...
                    <Link
                      href={item.href}
                      onClick={() => setMobileOpen(false)}
                      className="block py-3 font-outfit text-lg font-normal text-primary"
                    >
                      {item.label}
                    </Link>
                  )}
